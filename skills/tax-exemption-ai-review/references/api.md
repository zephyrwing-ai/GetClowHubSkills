...
This reference documents backend APIs used by the skill. The local helper script uses the Friday production backend and embedded Friday production backend key by default. Override the backend key at runtime through `--api-key` or `TAX_EXEMPTION_API_KEY` when needed.
...
The helper passes the backend API key in the request header:
...
For normal review runs, use the helper's `pending --all` option. It fetches
pages until the returned item count reaches `data.count`, a page is empty, or
`--max-pages` is reached. A response with `count: 0` and `list: null` means no
pending records were fetched and no audit review/writeback should be reported.
